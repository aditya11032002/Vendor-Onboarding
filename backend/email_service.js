...
/**
 * Sends customer invitation email using nodemailer.
 * @param {Object} params
 ...
 * @returns {Promise<{ emailSent: boolean, message: string }>}
 */
async function sendCustomerInvitation({ toEmail, generatedPassword, portalUrl }) {
  const smtpHost = process.env.SMTP_HOST;
  const smtpPort = process.env.SMTP_PORT || 587;
  const smtpUser = process.env.SMTP_USER;
  const smtpPass = process.env.SMTP_PASS;

  console.log(`[INVITATION CREATED] Customer Email: ${toEmail} | Temporary Password: ${generatedPassword}`);
  console.log(`[SMTP CONFIG CHECK] Host: ${smtpHost || 'NONE'} | Port: ${smtpPort || 'NONE'} | User: ${smtpUser || 'NONE'}`);

  if (!smtpHost || !smtpUser || !smtpPass) {
    return {
      emailSent: false,
      message: 'Customer user account created, but SMTP is not configured. Credentials printed to server logs.'
    };
  }
  ...
  try {
    ...
    return {
      emailSent: false,
      message: 'Customer registered, but SMTP connection/auth failed. Check server logs.'
    };
  }

  const mailOptions = {
    from: `"VK18 Customer Portal" <${smtpUser}>`,
    to: toEmail,
    subject: 'Welcome to VK18 Customer Portal - Onboarding Invitation',
    html: `
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e2e8f0; border-radius: 8px;">
        <h2 style="color: #10b981; margin-bottom: 20px;">VK18 Pvt Ltd - Customer Onboarding</h2>
        <p>Hello,</p>
        <p>You have been invited to register as a partner/customer on the VK18 Portal.</p>
        <p>Please use the credentials below to log in and fill out the Customer Registration Form:</p>

        <div style="background-color: #f8fafc; padding: 15px; border-radius: 8px; margin: 20px 0; border-left: 4px solid #10b981;">
          <p style="margin: 5px 0;"><strong>Portal URL:</strong> <a href="${portalUrl}" style="color: #10b981; text-decoration: underline;">${portalUrl}</a></p>
          <p style="margin: 5px 0;"><strong>Username (Email):</strong> <code>${toEmail}</code></p>
          <p style="margin: 5px 0;"><strong>Temporary Password:</strong> <code>${generatedPassword}</code></p>
        </div>
  ...
  try {
    const info = await transporter.sendMail(mailOptions);
    console.log(`[EMAIL DISPATCHED SUCCESS] Customer invitation email sent to ${toEmail}. MessageId: ${info.messageId}`);
    return {
      emailSent: true,
      message: 'Customer registered and invitation email sent.'
    };
  } catch (mailError) {
    console.error(`[SMTP ERROR FAILURE] Failed to send email to ${toEmail}:`, mailError.message || mailError);
    return {
      emailSent: false,
      message: 'Customer registered, but the invitation email failed to send. Check server logs.'
    };
  }
}
